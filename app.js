...
setInterval(function(){ alert("Time up"); }, 15000);
  ...
  const switchLanguage = language => (language === 'ру' ? 'Maths' : 'Maths')

  //const heroMessage = language => (language === 'ру' ? 'Welcome' : 'cout')
  const heroMessage=language => 'Welcome'
 ...
 /*   language: () => state => {
  ...
  const correctAnswers = (language, n) =>
    (language === 'ру' ? `correct ${n}` : `correct ${n}`)
